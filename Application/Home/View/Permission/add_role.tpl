							...
							<?php if(isset($value['son'])) :?>
								<td><dl>
									<?php foreach($value['son'] as $k => $v) :?>
										<dd style="text-align:left"><input class="per_checkbox" parent_id="<?php echo $v['parent_id'] ?>" name="per[]" type="checkbox" value="<?php echo $v['id'] ?>" <?php if(in_array($v['id'],explode(',', $mof_info['permission_ids']))) echo 'checked';?> /> <?php echo $v['name'] ?> </dd>
									<?php endforeach; ?></dl>
								</td>
							<?php endif; ?>
						</tr>
