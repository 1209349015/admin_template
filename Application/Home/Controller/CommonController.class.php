...
*/
			...
			if(!$info)
				$this->error('服务器查询该管理员信息失败，服务器严重失败');
		...
		*/
		...
		/*
			...
			@param 		id 文件id
						type 1,缩略图；2原图
		*/
		protected function _getImgById($id,$type=1) {
			$file_model 	= M('file');
			$file_info 		= $file_model->field('file_dir,thumb_img_dir')->where(array('file_id' => $id))->find();
			
			if($file_info === false)
				$this->error('服务器查询失败');
			else if($file_info === null)
				$this->error('服务器获取该图片失败');
		...
		*/
		protected function _getArea() {
			$info 					= S($this->_areaName);

			if($info === false) {
				$region_model 			= M('region');
				$region_info			= $region_model->field('id,name,parent_id')->select();
				if(!$region_info)
					$this->error('服务器读取地区信息失败');

				if(!S($this->_areaName,serialize($region_info),array('type' => 'file', 'expire' => 0)))
					$this->error('服务器缓存数据失败');
			}
